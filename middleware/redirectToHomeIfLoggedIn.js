...
const redirectToHomeIfLoggedIn = (req, res, next) => {
  ...
  if (!token) {
    next();
    // when using the below statement the page gets stuck into infinte loop of redirection
    // return res.redirect("/login");
  }
  
  // Checking if we are able to get user details, this means that user has already logged in
  try {
